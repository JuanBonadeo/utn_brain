...
- 2026-09-14: **machete del 2º parcial** en `materias/ASI/estudio/machete-parcial2.md` + `.pdf` (12 páginas). Está armado como instrumento de examen, no como resumen: cada tema arranca por *qué te piden* y sigue por *qué teoría hay que meter adentro de la resolución*, que es el formato que exigió el docente. Contenido: el organigrama de Bancorp de bolsillo con la **tabla de traducción rol ITIL → área de Bancorp**; el protocolo de incidentes en sus tres etapas con los seis pasos del registro, impacto/urgencia/prioridad y los cuatro pasos de cierre; una **tabla de derivación tipo-de-incidente → área de 2º nivel → escalado jerárquico** lista para copiar; las herramientas (KB, CMDB/CI, ITSM, RFC/CAB/ECAB, versiones, SLA) y los ocho KPI con la fórmula de redacción (nombre + fórmula + meta + frecuencia + responsable); un **esqueleto de respuesta** de siete puntos; el perfil de puesto y las cinco partes del anuncio con AIDA; y el barrido de H&S por los cuatro grupos de condiciones de trabajo con un **catálogo de trece riesgos del área de desarrollo**, cada uno con su medida de nivel 1–2 antes del EPP. Cada tema cierra con "lo que te baja puntos" y el documento con un checklist de 60 segundos. Se marcó con **[CG]** lo que es conocimiento general y no sale del material (ISO 45001, la cadena SLA→OLA→UC que el apunte no menciona).
...
- 2026-08-26: **recorte de extensión del entregable, de 53 a 44 páginas** (−17% de páginas, −23% de palabras), revisado contra la guía de cátedra y sin tocar un solo dato numérico. Cuatro hallazgos: (1) el **Bloque 6** pide *informar* siete ítems del cronograma —período de inicio, duración, camino crítico, actividades con holgura, personas por perfil, conflictos y ajustes—, no narrarlos: el punto 10 pasó de 7.507 a 2.474 palabras, con una tabla de síntesis al frente; (2) las 50 actividades estaban **listadas dos veces**, en la EDT del 4.3 y en la tabla de fechas del 10, así que esta última se redujo a las 20 con holgura, ya que el camino crítico va como secuencia y el diagrama existe como figura 1; (3) el **Bloque 9** muestra los activos y su forma de adquisición en **una sola tabla**, de modo que los puntos 7 y 8 se fusionaron —al hacerlo, cinco de los once activos no emparejaban porque las dos tablas usaban nombres distintos para el mismo ítem, y hubo que mapearlos por coincidencia de prefijo—; (4) el **Bloque 7** presenta los perfiles como tabla, así que las nueve subsecciones por perfil del punto 5 se reemplazaron por una tabla perfil→paquetes generada desde la EDT, que además resultó **más precisa que la prosa**: las subsecciones citaban listas de paquetes incompletas. Se suprimió también la sensibilidad del punto 11 —la consigna sólo pide *indicar cuáles variables*— y se comprimió la especificación de los planos, que era un instructivo para dibujarlos y quedó obsoleta cuando Nacho los dibujó. **Error de fondo corregido:** el documento decía 51 actividades en cinco lugares y son **50**; en consecuencia, 20 con holgura y no 21. Se consolidó `etapa3-v2` en `etapa3`, eliminando la copia anterior.
- 2026-08-30: **correcciones del docente sobre la Etapa 3 entregada.** El balance fue muy positivo —destacó la conexión con las etapas anteriores, la delimitación del proyecto, la defensa del ciclo de vida híbrido y el nivel de detalle de EDT, RRHH, higiene y seguridad, adquisiciones, RFI/RFP, aplanamiento, costos y factibilidad— y marcó tres inconsistencias, las tres reales. **(1) Cantidad de actividades:** el documento decía "cuarenta y seis paquetes" en la EDT y el generador del anexo "51 actividades"; el número verificado contra `scripts/cpm-edt.py` es **50**, corregido en ambos y regenerado el anexo. **(2) Cantidades del punto 7 contra el costeo:** el punto 7 estaba dimensionado sobre 180 técnicos —229 licencias, 190 dispositivos, 190 MDM— y los puntos 11 y 12 sobre 60 técnicos —78 licencias, 63 dispositivos—. Es el mismo tipo de error que ya se había reconciliado entre el 11 y el 12 el 23/08: al unificar entonces se tocaron esos dos puntos pero no la tabla del 7. Se unificó en la cifra del costeo, que es la que además sostiene el cálculo de beneficios del punto 12, y se agregó la sección **Alcance dimensionado** explicitando que todo corresponde al Gran Rosario y no a la operación nacional, que es lo que el docente pedía. **(3) Corrimientos mayores que la holgura en 5.6, 7.5 y 6.2:** no había error de cálculo, y la hipótesis del docente era la correcta. El cronograma aplanado termina en 192 días y no en 187, de modo que las fechas tardías se recalculan y toda cadena no crítica gana cinco días de holgura. Se agregó a la tabla de corrimientos la columna **Holgura a 192** junto a la original, más el párrafo que lo explica. Verificado que ninguna de las 24 actividades corridas excede su holgura en la red aplanada; 6.2 queda exactamente en el límite, con 54 de corrimiento contra 54 de holgura recalculada. Documento en 45 páginas.
